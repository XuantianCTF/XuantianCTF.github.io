...
这一篇把 CTF 里绕不开的计算机基础过一遍，主要给 Reverse 和 PWN 打底。下面是知识点的集合，不是完整教程，遇到不熟的地方回来查就行。

### 为什么需要这些基础

Reverse 和 PWN 的题都建在"机器是怎么运行的"之上。寄存器搞不懂，汇编就白看；栈的布局不清楚，栈溢出根本无从下手。先把这些概念认熟，后面用到的时候才不会卡壳。
...
寄存器是 CPU 内部的高速存储单元，用来临时放指令、数据和地址。
...
栈是程序运行时存放局部变量、函数参数和返回地址的内存区域，遵循"后进先出"（LIFO）。
...
堆是程序运行时动态分配内存的区域，由 `malloc`/`free` 或 `new`/`delete` 管着。
...
堆利用在 PWN 里属于进阶内容，不同版本分配器的行为不一样，glibc 2.23/2.27/2.31/2.35 都需要了解。
...
IDA 的 F5 能把汇编还原成类 C 的伪代码，看懂这层对应关系，逆向会快很多：
...
Tux担心大家把基础概念搞混，`贴心(?)`地附上了最新的 SystemV amd64 文档～
