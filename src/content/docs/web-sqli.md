...
SQL 注入（SQL Injection）是一种常见的 Web 安全漏洞。攻击者在输入字段中插入 SQL 代码，欺骗后台数据库执行非预期的语句，比如绕过登录、读取或篡改数据。这类漏洞之所以存在，是因为应用把用户输入直接拼进了 SQL 查询，而没有做充分的验证和过滤。

从安全角度看，SQL 注入的根本原因是"信任边界"的混淆，也就是应用程序错误地信任了用户提供的数据。
