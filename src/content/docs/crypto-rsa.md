...
RSA 是 Rivest-Shamir-Adleman 算法的缩写，一种非对称加密算法，也是使用最广的公钥加密算法之一。它之所以安全，是因为把两个大素数相乘很容易，但把乘积分解回素数却很难。RSA 的安全性就依赖"大数分解"这个数学难题，目前还没有已知的多项式时间算法能分解足够大的整数。在 CTF 密码学方向，RSA 题目非常常见。
