...
Linux 是 CTF 和网络安全领域最重要的操作系统之一。几乎所有安全工具都优先支持 Linux，大部分 CTF 比赛的服务端也跑在 Linux 上，掌握它算是学安全的基本功。
...
很多人把 Linux 称为"Linux 操作系统"，但严格来说，Linux 只是一个**内核**（Kernel），也就是操作系统最核心的部分。
...
- **2003 年**：Knoppix 发行版首次把"Live CD"概念带入 Linux，不用安装就能运行完整系统，这也成了后来 Kali Linux 的雏形
...
有趣的是，微软，这个曾经 Linux 最大的反对者，现在已经是 Linux 内核的第二大企业贡献者。
