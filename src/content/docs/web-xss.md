...
XSS（Cross-Site Scripting）是一种把恶意脚本注入到网页中的攻击方式。攻击者用 XSS 窃取用户 Cookie、发起钓鱼攻击等。它的本质是代码注入：把恶意的 JavaScript 注入到网页里，当其他用户访问页面时，代码就在他们的浏览器中执行。名字里的"Cross-Site"容易引起误解，其实它跟"跨站"关系不大，更多是"跨站点的脚本注入"。
